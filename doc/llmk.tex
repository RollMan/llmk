...
\begin{clopt}{\sopt{s}, \lopt{replace-config-entry}}
Substitute the given entry with this argument for one on \code{llmk.toml} or
magic comments. The replacing entry should be \code{\meta{key}=\meta{value}}.
For example,
\begin{htcode}
llmk -r 'source=["another.tex"]'
\end{htcoe}
This would be useful if you use, for example, \code{subfiles} and want to specify a path to a source file to be built.
\end{clopt}
